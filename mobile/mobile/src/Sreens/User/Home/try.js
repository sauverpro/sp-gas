...
const HomePage = () => {
  ...
  const fetchProduct = async () => {
    ...
    axios({
      ...
      .catch((error) => {
        console.log("products arroe occured:", error);
      });
  };
  const fetchTarrif = async () => {
    ...
    axios({
      ...
      .then((response) => {
        ...
        console.log(tarrifs, "useState Tarriffs");
        // console.log(tarrifs._id, "idsss");
        // setId(response?.data?.data._id);
        // console.log(isid, "idassa");
        setIsLoading(false);
      })
      .catch((error) => {
  ...
  const handleAddToCart = async (productId) => {
    if (!productId) {
      ...
      return; // Prevent API call if the product ID is undefined or null
    }
    axios({
      method: "POST",
